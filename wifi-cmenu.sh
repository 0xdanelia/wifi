...
then
	...
	printf "wifi reset\n"
	printf "    Bring wifi interface down, then up again. User requires 'sudo' permissions.\n\n"
	exit 0
fi
...
then
	...
	then
		...
		WIFI_NETWORK=$( echo "$WIFI_KNOWN_LIST" | cmenu -i -p "Select wifi network: " )
		# Exit if user presses 'ESC' in cmenu
		if [[ -z $WIFI_NETWORK ]];
		then
			exit 0
		fi
...
then
	# Pipe list of devices to cmenu if there is more than one
	WIFI_DEVICE=$(
		echo "$WIFI_DEVICE_LIST" | cmenu -i -p "Select wifi interface: "
	)
	# Exit if user presses 'ESC' in cmenu
	if [[ -z $WIFI_DEVICE ]];
	then
	...
	# Skip cmenu if there is only one device
...
then
	...
	WIFI_NETWORK=$(
		...
		cmenu -i -p "$WIFI_DEVICE - Select wifi network: " | 
		sed 's/\x1b\[[0-9;]*m\*\?//g' |  # remove colors from selected item
		sed 's/ *\(psk\|open\|8021x\) \+\*\+//g' | 
		sed 's/^ *>\? *//g' | 
		sed 's/ *$//g'
	)
